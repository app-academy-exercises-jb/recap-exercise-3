...
def no_dupes(arr)
    unique = []
    arr.each { |ele| unique << ele unless unique.include?(ele) }
    unique
end

def no_consecutive_repeats?(arr)
    arr.each_with_index.any? { |ele, idx| arr[idx+1] && arr[idx] == arr[idx+1] }
end
...
def longest_streak(str)
    ...
    butts = chars.inject { |max, c| 
        c[1] > max[1] ? c : max
    }
    debugger
end
__END__
rspec ./spec/problems_spec.rb:25 # General Problems #char_indices(str) should return a hash where each key corresponds to the unique chars of a string, the value of which is an array containing the indices where the char is found
